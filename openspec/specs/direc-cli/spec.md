...
Define the CLI surface that bootstraps Direc configuration and runs configured pipelines.

## Requirements

### Requirement: `direc init` materializes explicit workspace configuration from detected context

The CLI SHALL detect repository context and write a concrete pipeline configuration to `.direc/config.json`.

#### Scenario: Repository facets bootstrap built-in config
...
- **AND** JavaScript repositories SHALL enable the built-in `complexity`, `graph-maker`, `cluster-builder`, and `bounds-evaluator` tools
- **AND** repositories with OpenSpec SHALL enable `openspecTasks`, `openspecSpecs`, `specConflict`, `openspec-task-feedback`, and `openspec-spec-conflicts`
